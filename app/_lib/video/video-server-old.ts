...
import { OfferCallReq, OfferCallResp, CheckCallReq, CheckCallResp, AcceptCallReq, AcceptCallResp, RejectCallReq, RejectCallResp, CheckAcceptReq, CheckAcceptResp, HangUpReq, HangUpResp, WebRTCMsgReq, WebRTCMsgResp, SavePushSubscriptionReq, SavePushSubscriptionResp, DeletePushSubscriptionReq, DeletePushSubscriptionResp, StoreMsgReq, StoreMsgResp, AuthenticatedVideoReq } from "./video-common-old";
...
interface WebRTCMsgDoc {
    ...
    senders: {
        [sender: string]: {
            /**
             * overall added including those already fetched, acknowledged and deleted
             */
            count: number;
            msg: string[];
        }
...
async function storeMsg(authenticatedUser: string, req: StoreMsgReq): Promise<ApiResp<StoreMsgResp>> {
    const client = await clientPromise;
    const db = client.db(dbName);

    const col = db.collection<WebRTCMsgDoc>('webRTCMsg');
    const senderPrefix = `senders.${req.sender}.`
    const countName = senderPrefix + 'count';
    const arrayName = senderPrefix + 'msg';
