...
async function pushNotify(authUser: string, req: PushNotifyReq): Promise<ApiResp<PushNotifyResp>> {
    ...
    if (doc != null) {
        ...
        try {
            const sendRes = await sendPushMessage(subscription, { caller: authUser, callee: req.callee });
        } catch (reason) {
